...
class DbtGraph:
    ...
    def load_via_dbt_ls_cache(self) -> bool:
        ...
        logger.debug(f"Trying to parse the dbt project using dbt ls cache {self.cache_key}...")
        if self.should_use_dbt_ls_cache():
            project_path = self.project_path

            cache_dict = self.get_dbt_ls_cache()
            if not cache_dict:
                logger.debug(f"Cosmos performance: Cache miss for {self.cache_key}")
                return False
            ...
            if dbt_ls_cache and not cache.was_project_modified(cache_version, current_version):
                ...
                logger.debug(f"Cosmos performance: Cache hit for {self.cache_key} - {current_version}")
                return True
        logger.debug(f"Cosmos performance: Cache miss for {self.cache_key} - skipped")
        return False
